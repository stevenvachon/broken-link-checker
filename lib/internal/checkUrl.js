...
*/
function checkUrl(link, baseUrl, options, callback, cache)
{
	...
	if (cache)
	{
		if (cache.contains(link.url.resolved))
		{
			cache.retrieve(link.url.resolved)(callback);
			return;
		}

		if (cache.isRetrieving(link.url.resolved))
		{
	...
	function(error, response)
	{
		...
		if (cache)
		{
			cache.store(link.url.resolved, function(cb){
				callHandler[execution](cb, link);
			});
		}

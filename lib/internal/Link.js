...
export const ORIGINAL_URL = Symbol("originalURL");      // The URL string as it was inputted
export const RESOLVED_URL = Symbol("resolvedURL");      // The `URL`, resolved with `RESOLVED_BASE_URL`
export const REBASED_URL = Symbol("rebasedURL");        // The `URL`, resolved with `REBASED_BASE_URL`
export const REDIRECTED_URL = Symbol("redirectedURL");  // The `URL`, after its last redirection, if any

export const RESOLVED_BASE_URL = Symbol("resolvedBaseURL");  // The base `URL`
export const REBASED_BASE_URL = Symbol("rebasedBaseURL");    // The base `URL`, resolved with `HTML_BASE_HREF`

export const HTML_INDEX = Symbol("htmlIndex");               // The order in which the link appeared in its document -- using max-level tag filter
export const HTML_OFFSET_INDEX = Symbol("htmlOffsetIndex");  // Sequential (gap-free) indices for skipped and unskipped links
export const HTML_LOCATION = Symbol("htmlLocation");         // Source code location of the attribute that the link was found within
export const HTML_SELECTOR = Symbol("htmlSelector");         // CSS selector for element in document
export const HTML_TAG_NAME = Symbol("htmlTagName");          // Tag name that the link was found on
export const HTML_ATTR_NAME = Symbol("htmlAttrName");        // Attribute name that the link was found within
export const HTML_ATTRS = Symbol("htmlAttrs");               // All attributes on the element
export const HTML_TEXT = Symbol("htmlText");                 // TextNodes/innerText of the element
export const HTML_TAG = Symbol("htmlTag");                   // The entire tag string
export const HTML_BASE_HREF = Symbol("htmlBaseHref");        // The document's `<base href>` value

export const HTTP_RESPONSE = Symbol("httpResponse");                      // The request response
export const HTTP_RESPONSE_WAS_CACHED = Symbol("httpResponseWasCached");  // If the response was from cache

export const IS_BROKEN = Symbol("isBroken");        // If the link was determined to be broken or not
export const IS_INTERNAL = Symbol("isInternal");    // If the link is to the same server as its base/document
export const IS_SAME_PAGE = Symbol("isSamePage");   // If the link is to the same page as its base/document
export const WAS_EXCLUDED = Symbol("wasExcluded");  // If the link was excluded due to any filtering

export const BROKEN_REASON = Symbol("brokenReason");      // The reason why the link was considered broken, if it indeed is
export const EXCLUDED_REASON = Symbol("excludedReason");  // The reason why the link was excluded from being checked, if it indeed was

const MEMOIZED_KEY_VALUE = Symbol(true);
...
export default class Link extends Map
{
	...
	toJSON()
	{
		return Array.from(super.entries()).reduce((result, [key, value]) =>
		{
...
{
	result[value] = MEMOIZED_KEY_VALUE;
	return result;
}, {});
...
Object.assign(Link, keysAsValues);
Object.freeze(Link);

...
function cli()
{
	...
	this.nopter.config(
	{
      ...
      {
        short: ["q"],
        info: "Quiet mode - only show broken links.",
        type: Boolean
      },
			"host-requests":
			{
...
function logMetrics(quiet, brokenLinks, excludedLinks, totalLinks, duration, preBreak, exit)
{
  ...
  if (!quiet || brokenLinks > 0) {
  	log(output);
  }

	if (exit === true)
	{
    console.log('logMetrics exit', {brokenLinks})
...
*/
function logMetrics_delayed(quiet, brokenLinks, excludedLinks, totalLinks, duration, preBreak, exit)
{
	setImmediate( function()
	{
		logMetrics(quiet, brokenLinks, excludedLinks, totalLinks, duration, preBreak, exit);
	});
}
...
function logResult(result, finalResult, quiet)
{
	var output = "";
...
*/
function logResults(data, quiet)
{
	...
	while (nextIsReady)
	{
		...
		if (result !== undefined)
		{
			...
			output = logResult(result, done, quiet);
...
*/
function logResults_delayed(data, quiet)
{
...
function run(url, checkerOptions, logOptions, quiet)
{
  ...
  var quietPageLogger = null

  var possibleLogPage = function (data, pageUrl) {
    data.total.pages ++
    if (quiet) {
      quietPageLogger = function () {
      ...
      if (quiet) {
		...
		page: function(error, pageUrl)
		{
			...
			else
			{
				...
				logMetrics_delayed(quiet, data.page.brokenLinks, data.page.excludedLinks, data.page.totalLinks);
			}
		},
		end: function()
		{
			...
			{
        console.log('Pages <= 0', data.total)
        process.exit(1);
      }
      else if (data.total.pages === 1)
      {
				console.log('Pages === 1', data.total);
				process.exit(data.page.done===true && data.total.brokenLinks===0 ? 0 : 1);
			}
			else if (data.total.pages > 1)
			{
				logMetrics_delayed(quiet, data.total.brokenLinks, data.total.excludedLinks, data.total.links, Date.now()-data.total.startTime, true, true);
			}
		}
	};

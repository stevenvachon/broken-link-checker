...
function cli()
{
	...
	this.nopter.config(
	{
		...
		options:
		{
			...
			"exclude-scheme":
			{
				rename: "excludedSchemes",
				info: "URI schemes to ignore. Can be used multiple times.",
				type: [String, Array],
				default: defaultOptions.excludedSchemes
			},
			"exclude-external":
			{
...
cli.prototype.input = function(args, showArgs)
{
	...
	{
		// TODO :: remove these when nopter's default values for Arrays are fixed
		if (args.excludedKeywords === undefined)
		{
			args.excludedKeywords = defaultOptions.excludedKeywords;
		}
		if (args.excludedSchemes === undefined)
		{
			args.excludedSchemes = defaultOptions.excludedSchemes;
		}
		
		run(args.input,
		{
			excludedKeywords:       args.excludedKeywords,
			excludedSchemes:        args.excludedSchemes,

...
log.result = /*(*/result/*, finalResult)*/ =>
{
	if (result.displayed)
	{
		...
		if (link.get(Link.IS_BROKEN))
		{
			output += red("BROKEN");
			output += gray("─ ");
		}
		else if (link.get(Link.WAS_EXCLUDED))
		{
			output += gray("─SKIP── ");
		}
		else
		...
		output += yellow( link.get(Link.REBASED_URL) ?? link.get(Link.ORIGINAL_URL) );

		if (link.get(Link.IS_BROKEN))
		{
			output += gray(` (${link.get(Link.BROKEN_REASON)})`);
		}
		else if (link.get(Link.WAS_EXCLUDED))
		{
			output += gray(` (${link.get(Link.EXCLUDED_REASON)})`);
		}
		// Don't display cached message if broken/excluded message is displayed
		else if (link.get(Link.HTTP_RESPONSE_WAS_CACHED))
		{
			output += gray(" (CACHED)");
		}
...
class Statistics
{
	...
	pushResult(link)
	{
		...
		const hideCachedLink   = logOptions.hideCachedLinks   && link.get(Link.IS_BROKEN)===false && link.get(Link.HTTP_RESPONSE_WAS_CACHED);
		const hideSkippedLink  = logOptions.hideSkippedLinks  && link.get(Link.WAS_EXCLUDED);
		const hideUnbrokenLink = logOptions.hideUnbrokenLinks && link.get(Link.IS_BROKEN)===false;
		...
		{
			this.page.results[link.get(Link.HTML_INDEX)] = result;
		}
		else
		{

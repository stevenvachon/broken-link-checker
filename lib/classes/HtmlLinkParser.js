...
HtmlLinkParser.prototype.parse = function(html, base)
{
	reset(this);
	
	this.parserVars.urlBase = base;
...
function getLinks(tagName, attrs, instance)
{
	...
	for (attrName in attrMap)
	{
		...
		// TODO :: this is ran twice -- here and in checkUrl
		linkObj.resolve(link, _.urlBase, _.options);
		
		// If link should be ignored, skip
		if (link.internal===true && _.options.excludeInternalLinks===true) continue;
		if (link.samePage===true && _.options.excludeLinksToSamePage===true) continue;
		if (_.options.excludedSchemes[link.url.parsed.protocolTruncated] === true) continue;

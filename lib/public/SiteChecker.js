...
import {EXCLUDED_REASON, HTML_ATTR_NAME, HTML_TAG_NAME, HTTP_RESPONSE, IS_BROKEN, IS_LOCAL, REBASED_URL, REDIRECTED_URL, WAS_EXCLUDED} from "../internal/Link";
...
export default class SiteChecker extends SafeEventEmitter
{
	...
	{
		if (link.get(IS_LOCAL) && !this.#isAllowed(link))
		{
			return "BLC_ROBOTS";
		}
		else
	...
	#maybeEnqueuePage(link, customData, auth)
	{
		...
		else if (redirectedURL !== null)
		{
			...
			link.get(HTTP_RESPONSE).redirects.forEach(redirect => this.#sitePagesChecked.set(redirect.url, PAGE_WAS_CHECKED));

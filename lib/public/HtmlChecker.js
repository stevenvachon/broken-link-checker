...
import {HTML_ATTR_NAME, HTML_ATTRS, HTML_INDEX, HTML_OFFSET_INDEX, HTML_TAG_NAME, IS_LOCAL, IS_SAME_PAGE, REBASED_URL} from "../internal/Link";
...
export default class HtmlChecker extends SafeEventEmitter
{
	...
	#getExcludeReason(link)
	{
		...
		const isLocal = link.get(IS_LOCAL);
		...
		else if (excludeLocalLinks && isLocal)
		{
			return "BLC_LOCAL";
		}
		...
		else if (excludeRemoteLinks && isLocal===false)
		{
			return "BLC_REMOTE";
		}

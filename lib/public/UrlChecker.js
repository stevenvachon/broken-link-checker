...
function UrlChecker(options, handlers)
{
	...
	},
	{
		item: function(input, done)
		{
			...
			{
				checkUrl(input.data.linkObj, null, thisObj.options, handle_checkUrl, thisObj.cache);
			}
			else
			{
				// TODO :: send url object -- remove orgUrl?
				checkUrl(input.data.orgUrl, input.data.baseUrl, thisObj.options, handle_checkUrl, thisObj.cache);
			}
		},
		end: function()
		{

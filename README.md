...
* Support for relative URLs with `<base>`.
...
[Node.js](http://nodejs.org) `>= 10` is required. There're two ways to use it:
...
A synchronous callback that is called after all other filters have been performed. Return `true` to include `link` (a [`Link`](https://github.com/stevenvachon/broken-link-checker/blob/v0.8.0/lib/internal/Link.js#L15-L55)) in the list of links to be checked, or return `false` to have it skipped.
...
[coveralls-image]: https://img.shields.io/coveralls/stevenvachon/broken-link-checker

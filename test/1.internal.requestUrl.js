...
describe("INTERNAL -- requestUrl", () =>
{
	...
	it("does not receive a HEAD stream", () =>
	{
		const auth = {};
		const options = helpers.options();
		const url = new URL("http://blc/normal/index.html");
	...
	it.skip("does not receive a PSEUDO-HEAD stream", () =>
	{
		const auth = {};
		const options = helpers.options();
		const url = new URL("http://blc/normal/index.html");

...
import request from "../lib/internal/http-protocol/request";
...
describe("INTERNAL -- request", () =>
{
	...
	it("returns a Promise", () =>
	{
		...
		const returnedValue = request(url, defaultAuth, GET, cache, parsedOptions());
	...
	{
		const url = new URL("http://blc/simple/index.html");
		const {response, stream} = await request(url, defaultAuth, GET, cache, parsedOptions());
	...
	{
		const url = new URL("http://blc/simple/index.html");
		const result = await request(url, defaultAuth, HEAD, cache, parsedOptions());
	...
	it("rejects an erroneous url", async () =>
	{
		...
		try
		{
			await request("/simple/404.html", defaultAuth, GET, cache, parsedOptions());
		}
		catch (error)
		{
			expect(error)
	...
	describe("caching", () =>
	{
		...
		resolves.forEach(({title, url}) => it(title, async () =>
		{
			...
			request(url, defaultAuth, GET, cache, options);
		...
		it("stores the response of a failed connection", async () =>
		{
			...
			try
			{
				await request(url, defaultAuth, GET, cache, options);
			}
			catch
			{
				errorWasThrown = true;
		...
		it("stores the response of redirected urls", async () =>
		{
			...
			request(url1, defaultAuth, GET, cache, options);
		...
		it("does not store the error from an erroneous url", async () =>
		{
			...
			try
			{
				await request("/simple/404.html", defaultAuth, GET, cache, options);
			}
			catch
			{
				errorWasThrown = true;
	...
	describe("options", () =>
	{
		it("retryHeadFail = false", async () =>
		{
			...
			const {response: {status}} = await request(url, defaultAuth, HEAD, cache, options);
		...
		it("retryHeadFail = false (#2)", async () =>
		{
			...
			const {response: {status}} = await request(url, defaultAuth, GET, cache, options);

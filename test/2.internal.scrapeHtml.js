...
describe("INTERNAL -- parseHtml / scrapeHtml", () =>
{
	...
	describe("edge cases", () =>
	{
		...
		it("supports multi-url attribute values", () =>
		{
		...
		it("supports irregular use of <base/>", () =>
		{
		...
		it("ignores invalid use of <base/>", () =>
		{
			let html = `<base>`;
			html += `<a href="file.html">link</a>`;
		...
		it("supports empty html documents", () =>
		{
			return wrapper("")
			.then(links => expect(links).to.be.empty)
			.then(() => wrapper(" "))

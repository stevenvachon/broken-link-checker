		...
		it("does not report site error when non-first page's html cannot be retrieved", done =>
		{
			...
			.on(PAGE_EVENT, error =>
			{
				...
				else
				{
					expect(error)
						.to.be.an("error")
						.to.be.an.instanceOf(HTMLRetrievalError);
				}
			})
			.on(SITE_EVENT, error =>
			{
			...
			.on(LINK_EVENT, link =>
			{
				...
				switch (linkCount++)
				{
					...
					case 5:
					{
						...
						expect(link.get(REBASED_URL).password).to.be.empty;
						expect(link.get(REBASED_URL).username).to.be.empty;
						//expect( parseAuth(link.get(HTTP_RESPONSE).headers.authorization) ).to.not.deep.equal({ name:"user", pass:"pass" });  // @todo https://github.com/sindresorhus/got/issues/1652
						break;
					}
					case 6:
					{

...
import Link, {EXCLUDED_REASON, HTML_INDEX, HTML_OFFSET_INDEX, HTML_TEXT, HTTP_RESPONSE, HTTP_RESPONSE_WAS_CACHED, IS_BROKEN, ORIGINAL_URL, REBASED_BASE_URL, REBASED_URL, REDIRECTED_URL, WAS_EXCLUDED} from "../lib/internal/Link";
...
describe("PUBLIC -- SiteChecker", () =>
{
	...
	describe("edge cases", () =>
	{
		...
		it("supports multiple queue items", done =>
		{
			...
			.on(END_EVENT, () =>
			{
				...
				expect(results[0][0][1].get(IS_BROKEN)).to.be.true;   // link -- fake.html
				...
				expect(results[0][1][1].get(IS_BROKEN)).to.be.true;   // link -- fake.html
				...
				expect(results[1][0][1].get(IS_BROKEN)).to.be.true;   // link -- fake.html
				...
				expect(results[1][1][1].get(IS_BROKEN)).to.be.true;   // link -- fake.html
		...
		it("supports pages behind basic auth", done =>
		{
			...
			.on(LINK_EVENT, link =>
			{
				...
				switch (linkCount++)
				{
					...
					{
						expect(link.get(IS_BROKEN)).to.be.true;
						expect(link.get(ORIGINAL_URL)).to.equal("fake.html");
						expect(link.get(REBASED_URL).password).to.be.empty;
						expect(link.get(REBASED_URL).username).to.be.empty;
						expect(link.get(HTTP_RESPONSE).headers.authorization).to.be.undefined;
					...
					{
						expect(link.get(IS_BROKEN)).to.be.true;
						expect(link.get(ORIGINAL_URL)).to.equal("http://user2:pass2@blc1/auth/fake.html");
						expect(link.get(REBASED_URL).password).to.equal("pass2");
						expect(link.get(REBASED_URL).username).to.equal("user2");
						expect(link.get(HTTP_RESPONSE).headers.authorization).to.be.undefined;
						expect(link.get(HTTP_RESPONSE).status).to.equal(404);
	...
	describe("options", () =>
	{
		...
		{
			const junkResults = [];
			const options = rawOptions({ customFilter: link => link.get(HTML_TEXT) !== "link-fake" });
			const results = [];
			let page = 0;

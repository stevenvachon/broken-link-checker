...
import Link, {HTML_BASE_HREF, IS_LOCAL, IS_SAME_PAGE, ORIGINAL_URL, REBASED_BASE_URL, REBASED_URL, REDIRECTED_URL, RESOLVED_BASE_URL, RESOLVED_URL} from "../lib/internal/Link";
...
describe("INTERNAL -- Link", () =>
{
	...
	describe("resolve()", () =>
	{
		...
		Object.entries(INTERPOLATED_URL_TESTS).forEach(([title, fixture]) =>
		{
			...
			{
				const {baseURL, htmlBaseURL, linkURL, local, rebasedBaseURL, rebasedLinkURL, resolvedBaseURL, resolvedLinkURL, samePage} = fixture;
				const link = new Link();
				...
				expect(link.get(IS_LOCAL)).to.equal(local);

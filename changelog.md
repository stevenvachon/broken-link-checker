* 0.8.0
  ...
  * options added: `includedKeywords`, `retryHeadCodes`
  * options changed: `acceptedSchemes`, `excludeLinksToSamePage`, `excludedSchemes`
  * options renamed: `cacheExpiryTime`→`cacheMaxAge`, `excludeExternalLinks`→`excludeRemoteLinks`, `excludeInternalLinks`→`excludeLocalLinks`, `retry405Head`→`retryHeadFail`
  * CLI options added: `--include`, `--verbosity`
  * CLI options removed: `--verbose`
  * CLI options renamed: `--exclude-external`→`--exclude-remote`, `--exclude-internal`→`--exclude-local`
